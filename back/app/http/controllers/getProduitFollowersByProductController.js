...
async function getProduitFollowersByProductMaria(email, lvl, productId, res) {
    try {
        ...
        `;
        // Paramètres : email, lvl, productId
        const params = [email, lvl, productId];
        ...
        console.error('Erreur dans getProduitFollowersByProductMaria:', error);
...
async function getProduitFollowersByProductNeo4j(email, lvl, productId, res) {
    ...
    try {
        ...
        const diff = process.hrtime(startTime);
        const durationSec = diff[0] + diff[1] / 1e9;
        res.status(200).json({ data: data, duration: durationSec });
    } catch (error) {
